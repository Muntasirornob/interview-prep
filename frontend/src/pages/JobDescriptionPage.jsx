...
function JobDescriptionPage({ onBack, onSubmit, savedPayload }) {
      ...
      <section className="job-description-card home-card mx-auto w-full max-w-3xl rounded-[2rem] border border-white/70 bg-white/80 px-5 py-10 shadow-[0_30px_90px_rgba(15,23,42,0.12)] backdrop-blur-sm sm:px-8 sm:py-14">
        <div className="text-center">
          ...
          <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
            Add the job title and description so the rewrite payload can be built from the session data.
          </p>

          {savedPayload ? (
            <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-left text-sm text-emerald-800">
              <p className="font-semibold">Saved for payload</p>
              <p className="mt-1">{savedPayload.job_title}</p>
            </div>
          ) : null}
        </div>
