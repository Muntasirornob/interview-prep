...
Extract ONLY what is explicitly stated or directly implied by specific project/experience descriptions in the resume. Do not infer or hallucinate skills.

If a project or role clearly requires a skill that is named (e.g. "built REST APIs" → REST APIs), extract it. If it is ambiguous, skip it.

Assign a confidence level to each skill:
...
Assign a proficiency signal based on context:
...
Do NOT include soft skills (e.g. "communication", "leadership") — those are handled by a separate chain.

Return your output as valid, minified JSON only. No preamble, no explanation, no markdown fences. Raw JSON only.
